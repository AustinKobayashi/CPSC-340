...
class DecisionStump:
    ...
    def fit(self, X, y):
        ...
        minError = np.sum(y != y_mode) #number of items != y_mode
        ...
        if np.unique(y).size > 1:
            ...
            for d in range(D):
                for n in range(N):
                    ...
                    #y[X[:,d] > value] is an array of y values when X[:,d] > value
                    #y_sat = most common value in y when X[:,d] > value
                    y_sat = utils.mode(y[X[:,d] > value]) 
                    
                    #y_not = most common value in y when X[:,d] <= value
