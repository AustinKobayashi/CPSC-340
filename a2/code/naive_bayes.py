...
class NaiveBayes:
    ...
    def fit(self, X, y):
        ...
        p_xy = np.zeros((D, C))
        for i in range(4):
            #p_xy[:,i] = X[np.where(y == i)[0]].sum(axis=0) / counts[i]
            #Add beta to numerator, and add beta for each possible label to denominator.
            p_xy[:,i] = (X[np.where(y == i)[0]].sum(axis=0) + self.beta) / (counts[i] + self.beta * C)
